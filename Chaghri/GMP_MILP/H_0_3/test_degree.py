...
class Chaghri:
    ...
    def generate_vars(self, n):
        ...
        for r in range(nr):
            for i in range(3):
                for j in range(n):
                    command += f"y0_{r}_{i}_{j}\n"
        for r in range(nr):
            for i in range(3):
                for j in range(n):
                    command += f"y1_{r}_{i}_{j}\n"
        for r in range(nr):
            for i in range(3):
                for j in range(n):
                   
                    command += f"yqq_{r}_{i}_{j}\n"
                    
        for r in range(nr):
            for i in range(3):
                for j in range(n+1):
                    command += f"yg_{r}_{i}_{j}\n"
                    command += f"ygg_{r}_{i}_{j}\n"
                   
        
        for r in range(nr):
            for i in range(3):
                for j in range(n):
                    command += f"yy0_{r}_{i}_{j}\n"
        for r in range(nr):
            for i in range(3):
                for j in range(n):
                    command += f"c0_{r}_{i}_{j}\n"
        
        for r in range(nr):
            for i in range(3):
                for j in range(n):
                    command += f"yy1_{r}_{i}_{j}\n"
        for r in range(nr):
            for i in range(3):
                for j in range(n):
                    command += f"c1_{r}_{i}_{j}\n"
        for r in range(nr):
            for i in range(3):
                for j in range(n):
        ...
        for r in range(nr):
            for i in range(3):
                for j in range(n):
                    command += f"p_qq0_{r}_{i}_{j}\n"
        for r in range(nr):
            for i in range(3):
                for j in range(n+1):
                    command += f"p_g0_{r}_{i}_{j}\n"
                    command += f"p_gg0_{r}_{i}_{j}\n"
       
        for r in range(nr):
            for i in range(3):
                for j in range(n):
                    command += f"p_qq1_{r}_{i}_{j}\n"
        for r in range(nr):
            for i in range(3):
                for j in range(n+1):
                    command += f"p_g1_{r}_{i}_{j}\n"
                    command += f"p_gg1_{r}_{i}_{j}\n"
        return command
    ...
    def xor3_constraint(self, xor_in, xor_out, command):
        ...
        # command += " - {} = 0\n".format(xor_out)
        return command
    def copy2_constraint(self,copy_in, copy_out, r, s, command):
        command += "yg_{}_{}_{} = 0\n".format(r, s, 0)
        # for i in range(3):
        #     command += "ggg_{}_{}_{} = 0\n".format(r, s, i)
        for i in range(self.n_block):
            command += f"2 yg_{r}_{s}_{i+1} + yqq_{r}_{s}_{i} - {copy_out[0]}_{i} - {copy_out[1]}_{i}  - yg_{r}_{s}_{i}  = 0\n"
            # 4*ggg[i+2]+2*g[i+1]+qq[i]==x[i]+y[i]+g[i]+z[i]+ggg[i]
        # gg[0]==g[63]
        command += f"ygg_{r}_{s}_0 - yg_{r}_{s}_63 = 0\n"

        for i in range(self.n_block):
            
            command += f"2 ygg_{r}_{s}_{i+1} + {copy_in}_{i} - yqq_{r}_{s}_{i} - ygg_{r}_{s}_{i} = 0\n"
    ...
    def power_constraint0(self, power_in, power_out, r, s, k0, command):
        k2 = self.k2
        command += f"p_g0_{r}_{s}_0 = 0\n"
        
        for i in range(self.n_block):
            command += f"2 p_g0_{r}_{s}_{i+1} + p_qq0_{r}_{s}_{i} - {power_out}_{(i-k2)%self.n_block} - {power_out}_{(i-k0-k2)%self.n_block} - p_g0_{r}_{s}_{i} = 0\n"
            
        command += f"p_gg0_{r}_{s}_0 - p_g0_{r}_{s}_63 = 0\n"
        
        for i in range(self.n_block):
            command += f"2 p_gg0_{r}_{s}_{i+1} + {power_in}_{i} - p_qq0_{r}_{s}_{i} - p_gg0_{r}_{s}_{i} = 0\n"
            
        return command

    def power_constraint1(self, power_in, power_out, r, s, k0, command):
        k3 = self.k3
        command += f"p_g1_{r}_{s}_0 = 0\n"
        
        for i in range(self.n_block):
            command += f"2 p_g1_{r}_{s}_{i+1} + p_qq1_{r}_{s}_{i} - {power_out}_{(i-k3)%self.n_block} - {power_out}_{(i-k0-k3)%self.n_block} - p_g1_{r}_{s}_{i} = 0\n"
            
        command += f"p_gg1_{r}_{s}_0 - p_g1_{r}_{s}_63 = 0\n"
        
        for i in range(self.n_block):
            command += f"2 p_gg1_{r}_{s}_{i+1} + {power_in}_{i} - p_qq1_{r}_{s}_{i} - p_gg1_{r}_{s}_{i} = 0\n"
            
        return command
    def and_bx(self,and_in,and_inn,command):
        for i in range(self.n_block):
            command+=f"{and_in}_{i} - {and_inn}_{i} = 0\n"
        return command
    ...
    def make_milp_model(self, testweight):
        ...
        # =======y copy to y0 and y1
                copy_in=f"y_{r}_{i}"
                copy_out=[f"y0_{r}_{i}",f"y1_{r}_{i}"]
                lp_content=self.copy2_constraint(copy_in,copy_out,r,i,lp_content)
        # =======power+affine1=======
                power_in = f"y0_{r}_{i}"
                power_out = f"yy0_{r}_{i}"
                lp_content = self.power_constraint0(
                    power_in, power_out, r, i, self.k0, lp_content)
        # =======power+affine2=======
                power_in = f"y1_{r}_{i}"
                power_out = f"yy1_{r}_{i}"
                lp_content = self.power_constraint1(
                    power_in, power_out, r, i, self.k0, lp_content)
        # ======and =========
                and_in = f"yy0_{r}_{i}"
                and_inn = f"c0_{r}_{i}"
                lp_content=self.and_bx(and_in,and_inn,lp_content)
        ...
        for r in range(self.nrounds):
            for i in range(3):
                xor_in = [f"yy0_{r}_{i}", f"yy1_{r}_{i}"]
                xor_out = f"z_{r}_{i}"
                lp_content = self.xor2_constraint(xor_in, xor_out, lp_content)
        # ========copy===============
                copy_in = f"z_{r}_{i}"
    ...
    def solve(self, testweight):
        ...
        # milp_model.setParam('Threads', 16)
        ...
        objval = int(round(milp_model.objVal))
